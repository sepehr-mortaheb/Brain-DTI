...
for metric in metrics:
    ...
    fig.savefig(
        op.join(res_dir, f'{atlas}/FC/Graph/Graph_{metric}_{atlas}_graph-{graph}.pdf'),
        dpi=300,
        bbox_inches='tight'
    )
    fig.savefig(
        op.join(res_dir, f'{atlas}/FC/Graph/Graph_{metric}_{atlas}_graph-{graph}.png'),
        dpi=300,
        bbox_inches='tight'
    )
...
file_addr = op.join(
    res_dir,
    f'{atlas}/FC/Graph/Statistics_Controls_nodalAUC_graph-{graph}.xlsx'
)
...
if len(sig_nodes) == 0:
    ...
    nb.save(
        ...
        op.join(
            res_dir,
            f'{atlas}/FC/Graph/Statistics_Controls_nodalAUC_{metric}_graph-{graph}.nii'
        )
    )
...
file_addr = op.join(
    res_dir,
    f'{atlas}/FC/Graph/Statistics_Cosmonauts_vs_Controls_nodalAUC_graph-{norm}.xlsx'
)
...
if len(sig_nodes) == 0:
    ...
    nb.save(
        ...
        op.join(
            res_dir,
            f'{atlas}/FC/Graph/Statistics_Cosmonauts_vs_Controls_nodalAUC_{metric}_graph-{norm}.nii'
        )
    )