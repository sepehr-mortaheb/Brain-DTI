...
res_dir = '/Users/sepehrmortaheb/MyDrive/Academic/LEIA/Projects/BRAIN-DTI/Cosmonauts_StructFunc/Analysis/results/SCH100/FC/'
df <- read.csv(
  file.path(
    res_dir, 
    'FC_Cosmonauts.csv'
...
fit_lmm <- function(data) {
  model <- tryCatch(
    lmer(val_z ~ time + (1 | subject), data = data),
...
write.xlsx(
  ...
  file.path(
    res_dir, 
    "MassUnivariate/FC_MassUnivariate_LMM.xlsx"
  ),
  overwrite = TRUE
)
